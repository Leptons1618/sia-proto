...
- [ ] Implement config loader from `config/default.toml`
- [ ] Add config file override support (env vars, CLI args)
- [ ] Validate configuration values on startup
- [ ] Add config hot-reload capability
- [ ] Support per-user config files (~/.sia/config.toml)

### 🔴 Storage Layer Enhancement
- [ ] Implement query methods (get_events, get_grants, get_audits)
...
- [ ] Implement actual Unix socket request handling
- [ ] Define IPC protocol/message format (JSON-RPC?)
- [ ] Handle client connections and disconnections
- [ ] Implement request routing (status, list, query, etc.)
...
### 🔴 CLI Enhancement
- [ ] Connect CLI to agent via IPC socket
- [ ] Implement `status` command with real agent data
  - [ ] Show uptime
  - [ ] Display memory/CPU usage
  - [ ] Show collector status
  - [ ] Display event counts by severity
...
- [ ] Add `show <event-id>` command for event details
...
- [ ] Implement channel-based event pipeline
  - [ ] Create mpsc channel for collector → analyzer
  - [ ] Implement event ring buffer (10k capacity)
  - [ ] Add backpressure handling
- [ ] CPU Collector improvements
  - [ ] Track per-core usage
  - [ ] Detect sustained high CPU (> 80% for 30s)
  - [ ] Calculate moving averages
  - [ ] Honor cpu_interval from config
- [ ] Memory Collector
  - [ ] Track system memory usage
  - [ ] Track swap usage
  - [ ] Detect memory pressure events
  - [ ] Track per-process memory (top 10)
- [ ] Process Collector
...
- [ ] Implement event analyzer stub
  - [ ] Receive events from collector channel
  - [ ] Apply rule-based analysis
  - [ ] Generate insights from patterns
- [ ] Rule Engine
  - [ ] Define rule format (YAML/TOML?)
  - [ ] Implement rule loader
  - [ ] Support threshold-based rules
...
- [ ] Response Processing
  - [ ] Parse LLM responses
  - [ ] Extract suggestions and remediation steps
  - [ ] Validate LLM outputs
  - [ ] Store suggestions in events table
- [ ] Fallback Support
